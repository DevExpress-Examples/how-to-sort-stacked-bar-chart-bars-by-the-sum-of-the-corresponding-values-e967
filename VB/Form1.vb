        ...
        Private Function GetAdditionalValues(ByVal point As SeriesPoint) As Double
            ...
            For i As Integer = 1 To chart.Series.Count - 1
                For j As Integer = 0 To chart.Series(i).Points.Count - 1
                    If Convert.ToDouble(chart.Series(i).Points(j).Argument) = Convert.ToDouble(point.Argument) AndAlso chart.Series(i).Points(j) IsNot point Then result += chart.Series(i).Points(j).Values(0)
                Next
            Next
        ...
        Private Sub SwapAdditionalPoints(ByVal pi As SeriesPoint, ByVal pj As SeriesPoint)
            For s As Integer = 1 To chart.Series.Count - 1
                ...
                For i As Integer = 0 To chart.Series(s).Points.Count - 1
                    If Convert.ToDouble(chart.Series(s).Points(i).Argument) = Convert.ToDouble(pi.Argument) Then swp1 = chart.Series(s).Points(i)
                    If Convert.ToDouble(chart.Series(s).Points(i).Argument) = Convert.ToDouble(pj.Argument) Then swp2 = chart.Series(s).Points(i)
                Next

                If swp1 IsNot Nothing AndAlso swp2 IsNot Nothing Then chart.Series(s).Points.Swap(swp1, swp2)
            Next
        End Sub
    End Class
End Namespace
